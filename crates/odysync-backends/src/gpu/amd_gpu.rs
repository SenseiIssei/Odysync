...
/// - `HKLM\SYSTEM\CurrentControlSet\Services\amdkmdag\Global\NVTweak\DisplayVersion`
/// - `HKLM\SOFTWARE\AMD\CN\DriverVersion`
#[cfg(windows)]
async fn read_installed_driver_version() -> Option<String> {
    let script = r#"
        $paths = @(
            @{Path='HKLM:\SYSTEM\CurrentControlSet\Services\amdkmdag\Global\NVTweak'; Name='DisplayVersion'},
