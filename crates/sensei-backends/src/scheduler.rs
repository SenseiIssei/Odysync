...
//! The scheduled command is `sensei apply --yes` (or the user's custom args).
...
pub const DEFAULT_TASK_NAME: &str = "SenseisUpdater";
...
pub struct ScheduleSpec {
    ...
    /// Extra arguments to pass to `sensei`.
...
fn plist_path(task_name: &str) -> PathBuf {
    ...
    PathBuf::from(home)
        ...
        .join(format!("dev.sensei.{task_name}.plist"))
}
...
pub async fn create_schedule(spec: &ScheduleSpec) -> Result<()> {
    ...
    let label = format!("dev.sensei.{}", spec.task_name);
...
pub async fn create_schedule(spec: &ScheduleSpec) -> Result<()> {
    ...
    let service_name = format!("dev.sensei.{unit}.service");
    let timer_name = format!("dev.sensei.{unit}.timer");
    ...
    let service = format!(
        "[Unit]\nDescription=Sensei's Updater scheduled run\n\n[Service]\nType=oneshot\nExecStart={exec_start}\n"
    );
    ...
    let timer = format!(
        "[Unit]\nDescription=Run Sensei's Updater {freq}\n\n[Timer]\nOnCalendar={on_calendar}\nPersistent=true\n\n[Install]\nWantedBy=timers.target\n",
        freq = spec.frequency,
    );
...
pub async fn remove_schedule(task_name: &str) -> bool {
    ...
    let timer_name = format!("dev.sensei.{task_name}.timer");
    let service_name = format!("dev.sensei.{task_name}.service");
...
pub async fn schedule_exists(task_name: &str) -> bool {
    ...
    dir.join(format!("dev.sensei.{task_name}.timer")).exists()
}
