...
///
/// Contents:
///   - `env.txt` — OS, sensei version, elevation status
...
pub async fn create_diagnostics(
    ...
    zip.start_file("env.txt", options)
        .map_err(|e| sensei_core::error::Error::Io(std::io::Error::other(e.to_string())))?;
...
mod tests {
    ...
    async fn creates_a_valid_zip() {
        let dir = std::env::temp_dir().join("sensei-diag-test");
