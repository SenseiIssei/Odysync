...
impl Config {
    ...
    /// `%APPDATA%\SenseisUpdater\config.json` or `~/.config/senseis-updater/config.json`.
    pub fn default_path() -> Result<PathBuf> {
        let dirs = directories::ProjectDirs::from("dev", "SenseiIssei", "SenseisUpdater")
...
mod tests {
    ...
    fn corrupt_file_is_an_error_not_a_silent_reset() {
        let dir = std::env::temp_dir().join("sensei-cfg-test-corrupt");
    ...
    fn round_trips_through_disk() {
        let dir = std::env::temp_dir().join("sensei-cfg-test-roundtrip");
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.json");
